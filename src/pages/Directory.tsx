...
export default function Directory() {
  ...
  const specLabel = spec === 'any' ? 'أي تخصص' : specialties.find((s) => s.id === spec)?.name_ar || 'تخصص'
  const countryLabel = country === 'any' ? 'أي دولة' : country
  ...
  const seats = useMemo(() => {
    if (q.trim()) return []
    const where = country === 'any' ? 'أي دولة عربية' : country
    const roles = spec === 'any' ? SEAT_ROLES : [specLabel]
    return roles
  ...
  return (
    <PageShell>
      <div className="flex flex-col gap-4 md:gap-5 md:px-2">
        <span className="text-xs md:text-[13px]" style={{ color: '#5C5C59' }}>دليل صنّاع الإنتاج في العالم العربي</span>
        <h1 className="m-0 font-semibold text-[28px] md:text-[44px]" style={{ lineHeight: 1.6, letterSpacing: '-0.01em' }}>
          ...
          لإنتاج{' '}
          <button type="button" onClick={() => setLen(LENS[(lenIdx + 1) % LENS.length].key)} className={tokenStyle} style={{ color: '#2563EB', borderColor: '#BFD0F7', fontSize: 'inherit' }}>{LENS[lenIdx].label}</button>
        </h1>

        {specOpen && (
          <Card className="p-5 md:p-6 flex flex-col gap-3" style={{ boxShadow: '0 12px 32px rgba(0,0,0,0.08)' }}>
            <span className="text-[13px] font-semibold">اختر التخصص</span>
            <div className="flex flex-wrap gap-2">
              <Chip on={spec === 'any'} onClick={() => { setSpec('any'); setSpecOpen(false) }}>كل التخصصات</Chip>
              {specialties.map((s) => (
                <Chip key={s.id} on={spec === s.id} onClick={() => { setSpec(s.id); setSpecOpen(false) }}>{s.name_ar || s.name_en}</Chip>
              ))}
            </div>
          </Card>
        )}
        {countryOpen && (
          <Card className="p-5 md:p-6 flex flex-col gap-3" style={{ boxShadow: '0 12px 32px rgba(0,0,0,0.08)' }}>
            <span className="text-[13px] font-semibold">اختر الدولة</span>
            <div className="flex flex-wrap gap-2">
              <Chip on={country === 'any'} onClick={() => { setCountry('any'); setCountryOpen(false) }}>كل الدول العربية</Chip>
              {ARAB_COUNTRIES.map((c) => (
                <Chip key={c} on={country === c} onClick={() => { setCountry(c); setCountryOpen(false) }}>{c}</Chip>
              ))}
            </div>
          </Card>
        )}

        <p className="m-0 text-[13px] md:text-sm" style={{ color: '#5C5C59' }}>اضغط على أي كلمة باللون الأزرق لتغييرها. جميع الصنّاع هنا راجعهم فريق Makers.</p>
        <label className="flex items-center gap-2.5 h-12 md:h-[52px] px-5 rounded-full" style={{ border: '1px solid #E3E3E0', background: '#F7F7F6' }}>
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="أو ابحث باسم شخص تعرفه" className="flex-1 min-w-0 bg-transparent" style={{ border: 'none', height: '100%', fontSize: 15, padding: 0 }} />
        </label>
      </div>
      ...
      <Card className="px-5 md:px-8 py-2">
        <div className="flex justify-between items-baseline pt-5 pb-3.5" style={{ borderBottom: '1px solid #111' }}>
          <span className="text-[13px] font-semibold">الصنّاع</span>
          <span className="text-xs mono" style={{ color: '#5C5C59' }}>
            {makers === null ? '…' : `الصنّاع: ${rows.length}${seats.length ? ` · مقاعد متاحة: ${seats.length}` : ''}`}
          </span>
        </div>

        {makers === null && <div className="py-12 text-center text-sm" style={{ color: '#5C5C59' }}>جارٍ التحميل…</div>}
        ...
        {shown.map((m, i) => (
          <Link key={m.id} to={`/${m.username || ''}`} className="flex items-center gap-3 md:gap-5 py-4 md:py-5" style={{ borderBottom: '1px solid #F0F0EE' }}>
            ...
            <span className="flex flex-col gap-1 min-w-0 flex-1 md:flex-none">
              ...
              <span className="md:hidden text-[13px]" style={{ color: '#3A3A38' }}>{roleLine(specialties, m.specialty_ids, m.other_specialty)} · {m.country}</span>
              {m.is_featured && <span className="self-start text-[11px] font-semibold px-1.5 rounded-full" style={{ color: '#B42318', border: '1.5px solid #D92D20' }}>اختيار الأسبوع</span>}
            </span>
            ...
            <span className="hidden md:block text-[13px] whitespace-nowrap w-[72px]" style={{ color: '#5C5C59' }}>{m.country}</span>
            <span className="flex items-center gap-1.5 text-xs w-auto md:w-16 whitespace-nowrap" style={{ color: m.available ? '#166534' : '#6F6F6C' }}>
        ...
        {seats.map((s, i) => (
          <Link key={s.role + i} to="/join" className="flex items-center gap-3 md:gap-5 py-4 md:py-5" style={{ borderBottom: '1px solid #F0F0EE' }}>
            ...
            <span className="flex flex-col gap-0.5 flex-1 md:flex-none">
              <span className="text-base md:text-xl font-medium" style={{ color: '#6F6F6C' }}>مقعد مؤسس</span>
            ...
            <span className="hidden md:block text-[13px] whitespace-nowrap w-[72px]" style={{ color: '#6F6F6C' }}>{s.where}</span>
            <span className="text-xs font-semibold md:w-16 whitespace-nowrap" style={{ color: '#2563EB' }}>احجز المقعد</span>
          </Link>
        ))}

        {makers !== null && rows.length === 0 && seats.length === 0 && (
          <div className="py-14 text-center text-[15px]" style={{ color: '#5C5C59' }}>
            لا يوجد صنّاع بهذه المواصفات بعد. <Link to="/join" className="font-semibold" style={{ color: '#2563EB' }}>كن أول المنضمّين</Link>
          </div>
        )}
        {rows.length > limit && (
          <div className="py-5 text-center">
            <button type="button" onClick={() => setLimit(limit + 30)} className="text-sm font-semibold cursor-pointer bg-transparent border-0" style={{ color: '#2563EB' }}>عرض المزيد</button>
          </div>
        )}
      </Card>
      ...
      <Card dark className="p-6 md:px-9 md:py-7 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex flex-col gap-1">
          <span className="text-[17px] md:text-lg font-bold">مقاعد الأعضاء المؤسسين محدودة</span>
          <span className="text-[13px]" style={{ color: '#BDBDB9' }}>يحصل أوائل المنضمّين إلى الدليل على شارة «عضو مؤسس» بشكل دائم.</span>
        </div>
        <Link to="/join" className="self-start md:self-auto text-sm font-semibold px-5 py-3 rounded-full" style={{ background: '#2563EB', color: '#fff' }}>احجز مقعدك</Link>
      </Card>
